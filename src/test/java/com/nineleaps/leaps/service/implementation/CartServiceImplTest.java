...
class CartServiceImplTest {
    ...
    @Test
    void updateCartItem_WhenCartItemDoesNotExist_ShouldThrowCartItemNotExistException() throws CartItemNotExistException, QuantityOutOfBoundException {
        // Arrange
        AddToCartDto addToCartDto = new AddToCartDto();
        User user = new User();
    ...
    @Test
    void updateCartItem_WhenCartItemNotExists_ShouldNotUpdateCartItem() throws CartItemNotExistException, QuantityOutOfBoundException {
        // Arrange
        AddToCartDto addToCartDto = new AddToCartDto();
    ...
    @Test
    void updateCartItem_WhenQuantityIsZero_ShouldDeleteCartItemFromCart() throws CartItemNotExistException, QuantityOutOfBoundException {
        // Arrange
        long productId = 1L;
        int initialQuantity = 2;
        AddToCartDto addToCartDto = new AddToCartDto();
        addToCartDto.setProductId(productId);
        addToCartDto.setQuantity(0); // Setting the quantity to zero
        User user = new User();

        Cart existingCartItem = new Cart();
        existingCartItem.setQuantity(initialQuantity);

        when(cartRepository.findByUserIdAndProductId(user.getId(), productId)).thenReturn(existingCartItem);

        // Act
        cartService.updateCartItem(addToCartDto, user);

        // Assert
        verify(cartRepository, times(1)).deleteById(existingCartItem.getId());
    }
